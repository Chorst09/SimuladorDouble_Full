...
import { VMCommissionTables, VMCommissionData } from './VMCommissionTables';

const VMCommissionsSection: React.FC = () => {
  // Default commission data based on the provided images
  const defaultCommissionData: VMCommissionData = {
    vendedor: [
  ...
  const [commissionData, setCommissionData] = useState<VMCommissionData>(defaultCommissionData);

  // Load saved data from localStorage on component mount
  useEffect(() => {
    const savedData = localStorage.getItem('vmCommissionsData');
    if (savedData) {
      try {
  ...
  const handleCommissionDataChange = (data: VMCommissionData) => {
    setCommissionData(data);
    localStorage.setItem('vmCommissionsData', JSON.stringify(data));
  };

  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-2xl font-bold mb-6">Precificação - Máquinas Virtuais</h1>
      <div className="mb-8">
...
export default VMCommissionsSection;
