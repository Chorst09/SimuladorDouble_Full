...
import { ManCommissionTables, ManCommissionData } from './ManCommissionTables';

const ManCommissionsSection: React.FC = () => {
  // Default commission data based on the provided images
  const defaultCommissionData: ManCommissionData = {
    vendedor: [
  ...
  const [commissionData, setCommissionData] = useState<ManCommissionData>(defaultCommissionData);

  // Load saved data from localStorage on component mount
  useEffect(() => {
    const savedData = localStorage.getItem('manCommissionsData');
    if (savedData) {
      try {
  ...
  const handleCommissionDataChange = (data: ManCommissionData) => {
    setCommissionData(data);
    localStorage.setItem('manCommissionsData', JSON.stringify(data));
  };

  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-2xl font-bold mb-6">Precificação - Internet MAN</h1>
      <div className="mb-8">
...
export default ManCommissionsSection;
