...
import { FiberCommissionTables, FiberCommissionData } from './FiberCommissionTables';

const FiberCommissionsSection: React.FC = () => {
  // Default commission data based on the provided images
  const defaultCommissionData: FiberCommissionData = {
    vendedor: [
  ...
  const [commissionData, setCommissionData] = useState<FiberCommissionData>(defaultCommissionData);

  // Load saved data from localStorage on component mount
  useEffect(() => {
    const savedData = localStorage.getItem('fiberCommissionsData');
    if (savedData) {
      try {
  ...
  const handleCommissionDataChange = (data: FiberCommissionData) => {
    setCommissionData(data);
    localStorage.setItem('fiberCommissionsData', JSON.stringify(data));
  };

  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-2xl font-bold mb-6">Precificação - Internet Fibra</h1>
      <div className="mb-8">
        <h2 className="text-xl font-semibold mb-4">Tabela de Comissões</h2>
        <FiberCommissionTables 
...
export default FiberCommissionsSection;
