...
import { RadioCommissionTables, RadioCommissionData } from './RadioCommissionTables';

const RadioCommissionsSection: React.FC = () => {
  // Default commission data based on the provided images
  const defaultCommissionData: RadioCommissionData = {
    vendedor: [
  ...
  const [commissionData, setCommissionData] = useState<RadioCommissionData>(defaultCommissionData);

  // Load saved data from localStorage on component mount
  useEffect(() => {
    const savedData = localStorage.getItem('radioCommissionsData');
    if (savedData) {
      try {
  ...
  const handleCommissionDataChange = (data: RadioCommissionData) => {
    setCommissionData(data);
    localStorage.setItem('radioCommissionsData', JSON.stringify(data));
  };

  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-2xl font-bold mb-6">Precificação - Internet Rádio</h1>
      <div className="mb-8">
        <h2 className="text-xl font-semibold mb-4">Tabela de Comissões</h2>
        <RadioCommissionTables 
...
export default RadioCommissionsSection;
