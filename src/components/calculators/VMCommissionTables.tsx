...
export interface VMCommissionData {
...
interface VMCommissionTablesProps {
  commissionData: VMCommissionData;
  onCommissionDataChange: (data: VMCommissionData) => void;
}

export const VMCommissionTables: React.FC<VMCommissionTablesProps> = ({ 
  commissionData, 
  onCommissionDataChange 
}) => {
  const [isEditing, setIsEditing] = useState(false);
  const [editableData, setEditableData] = useState<VMCommissionData>({ ...commissionData });
    ...
    <div className="space-y-6">
      <Card className="border-0 bg-gray-900 text-white">
        <CardHeader className="flex flex-row items-center justify-between pb-2">
          <CardTitle className="text-lg font-medium">Tabela de Comissões - Máquinas Virtuais</CardTitle>
          {!isEditing ? (
...
export default VMCommissionTables;
